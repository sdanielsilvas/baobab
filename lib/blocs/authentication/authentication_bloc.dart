import 'package:baobab_app/blocs/authentication/authentication_event.dart';
import 'package:baobab_app/blocs/authentication/authentication_state.dart';
...
class AuthenticationBloc extends Bloc<AuthenticationEvent, AuthenticationState> {
    ...
    if (event is LoginEvent) {
      ...
      try {
        ...
        if (_user != null) {
          ...
          yield AuthenticationState.unauthenticated(); // for some reason didn't authenticate with no error
      } catch (error) {
      ...
      try {
        final FirebaseUser _user = await _signup(event.fullName, event.email, event.password, event.passwordRepeated);
      ...
      try {
        ...
        yield AuthenticationState.information('Reset your password with the link in the email sent to you');
      } catch (error) {
  ...
  Future<FirebaseUser> _signup(String fullName, String email, String password, String passwordRepeated) {
    if (fullName == "")
      ...
      throw Exception('Email is empy');
    else if (password == '')
      throw Exception('Password is empty');
    else if (passwordRepeated == '')
      throw Exception('Repeat password is empty');
  ...
  Future<FirebaseUser> _login(String email, String password, [bool autoLogin = false]) async {
  ...
  void onSignup(
    ...
    dispatch(SignupEvent(fullName: fullName, email: email, password: password, passwordRepeated: passwordRepeated));
  }
